...
WebsocketStream.prototype.bindReadableStream = function(hapiServer, path) {
  ...
  wss.on('connection', function(ws) {
    ...
    mx.on('error', function () {
      ...
      this.statStreams = this.statStreams.filter(function (s) { return s !== statStream; });
      this.activityStreams = this.activityStreams.filter(function (s) { return s !== activityStream; });
    });
  });
};
